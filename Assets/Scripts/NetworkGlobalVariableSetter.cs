...
namespace Eclipse.Networking {
    public class NetworkGlobalVariableSetter : NetworkBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (!fcc)
            {
                fcc = NetworkManager.LocalClient.PlayerObject.GetComponent<FPSCharacterControl>();
                return;
            }

            if (IsServer || IsHost)
            {
                fcc.moveSpeed.Value = sv_moveForce;
                fcc.airAcceleration.Value = sv_airAccel;
                fcc.maxMoveSpeed.Value = sv_maxMoveSpeed;
                fcc.canWallRun.Value = sv_canWallRun;
                fcc.canDoubleJump.Value = sv_canDoubleJump;
            }
            else
            {
                sv_moveForce = fcc.moveSpeed.Value;
                sv_airAccel = fcc.airAcceleration.Value;
                sv_maxMoveSpeed = fcc.maxMoveSpeed.Value;
        ...
        {
            Debug.Log($"sv_moveForce:{sv_moveForce}" +
                $"clientMoveForce:{fcc.moveSpeed.Value}" +
                $"sv_airaccel:{sv_airAccel}" +
                $"clientAirAccel{fcc.airAcceleration.Value}" +
                $"sv_maxMoveSpeed:{sv_maxMoveSpeed}" +
                $"clientMaxMoveSpeed:{fcc.maxMoveSpeed.Value}" +
